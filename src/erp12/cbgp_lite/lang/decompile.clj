...
(defn get-fn-symbol
  ...
  (cond
    ...
    (let [arity-map (get ast-arity-aliasing ast-fn-name)
      ...
      (cond
        ;; Number aliasing with multi-argument functions
        (contains? ast-number-aliasing fn-symbol)
        (get-fn-symbol fn-symbol tag args task)

        ;; TMH - Handling map-vec vs map-str vs map-map
        (= fn-symbol 'map-temp)
        (get-fn-symbol fn-symbol tag (rest args) task)


        ;; If no type aliasing, it just needs fn-symbol
        :else
        fn-symbol))
  ...
  ;;  (println "Decomp Task: " task)
   (cond
    ...
    ;; Handle static method or invoke
     ...
     ;; Handle vars, for function symbols
     (= op :var)
     {:gene :var :name 'BROKEN}
    ...
    ;; Handle anonymous function abstraction
     ...
     ;; Handle defn, which turns into def
     (= op :def)
     (decompile-ast (-> ast
                        :init
                        :expr
